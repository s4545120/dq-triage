...
"""
    ...
    # ---- violation_sample: the bad rows themselves -------------------------
    # check_run says how many. This says which. The two together are what the
    # workbench renders, and this is the only place production values reach the
    # dq catalog — the 100-row cap is the AI-exposure boundary from the parent
    # architecture doc, not a performance setting. Do not raise it here.
    #
    # Samples are collected for ANY rule whose predicate matched rows, including
    # shadow rules: a shadow rule runs and gathers evidence, it just does not
    # raise. The fixture's final run has 23 such rules — 21 breaching plus 2
    # shadow — and this reproduces that.
    import csv as _csv
    heads = {}
    for short, fn in (("ctct_c", "mock_ctct_c_1000 (1).csv"),
                      ("subs_c", "mock_subs_c_1000 (1).csv")):
        fp = pathlib.Path.home() / "Downloads" / fn
        if fp.exists():
            with fp.open(newline="") as fh:
                heads[short] = set(next(_csv.reader(fh)))

    vs_tbl = f"{q}{a.prefix}results_violation_sample"
    cr_tbl = f"{q}{a.prefix}results_check_run"
    sblocks = []
    for _, r in cur.sort_values("rule_id").iterrows():
        if r.rule_id not in last.index:
            continue
        rule = next((x for x in fixture_rules.RULES if x.rule_id == r.rule_id), None)
        if rule is None or not getattr(rule, "sample_columns", None):
            continue
        sh, t = shape(r), tbl(r.target_table)
        key, cols = rule.key_column, list(rule.sample_columns)
        ...
        if sh == "cross_table":
            ...
            src = (f"SELECT * FROM (SELECT *, {r.rule_expr} AS __dup FROM {t})"
                   f"\n          WHERE __dup{scope} LIMIT 100")
        elif sh == "variance":
            src = (f"SELECT * FROM {t}"
                   f"\n          WHERE {r.rule_expr.replace('{table}', t)}{scope} LIMIT 100")
        else:
            src = f"SELECT * FROM {t}\n          WHERE ({r.rule_expr}){scope} LIMIT 100"

        sblocks.append(
            f"""-- {r.rule_id}  ({sh})
INSERT INTO {vs_tbl}
  (sample_id, result_id, run_id, rule_id, target_table, row_key, sample_row, captured_ts)
SELECT uuid(), cr.result_id, cr.run_id, '{r.rule_id}', '{t}',
       CAST(v.{key} AS STRING),
       to_json(struct({', '.join('v.' + c for c in cols)})),
       current_timestamp()
FROM   ({src}) v
CROSS  JOIN (SELECT result_id, run_id FROM {cr_tbl}
             WHERE run_id = '{run_id}' AND rule_id = '{r.rule_id}') cr;""")
...
{chr(10).join(chr(10) + b for b in sblocks)}
"""
    (OUT / "samples_insert.sql").write_text(smp)
