...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_config_rule_registry (
...
ALTER TABLE workspace.dq_triage.dq_config_rule_registry
...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_config_playbook (
...
ALTER TABLE workspace.dq_triage.dq_config_playbook
...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_results_check_run (
...
ALTER TABLE workspace.dq_triage.dq_results_check_run
...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_results_violation_sample (
...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_results_disposition (
...
ALTER TABLE workspace.dq_triage.dq_results_disposition
...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_config_cde_registry (
...
ALTER TABLE workspace.dq_triage.dq_config_cde_registry
...
CREATE TABLE IF NOT EXISTS workspace.dq_triage.dq_results_cde_profile (
...
ALTER TABLE workspace.dq_triage.dq_results_cde_profile
...
CREATE OR REPLACE FUNCTION workspace.dq_triage.dq_fn_is_blank_v1(x STRING)
RETURNS BOOLEAN
...
-- reads NOT workspace.dq_triage.dq_fn_is_valid_email_v1(EML_ID). NULL-propagating: a NULL address
...
CREATE OR REPLACE FUNCTION workspace.dq_triage.dq_fn_is_valid_email_v1(x STRING)
RETURNS BOOLEAN
...
CREATE OR REPLACE FUNCTION workspace.dq_triage.dq_fn_is_au_mobile_v1(x STRING)
RETURNS BOOLEAN
...
CREATE OR REPLACE FUNCTION workspace.dq_triage.dq_fn_is_sentinel_v1(x STRING)
RETURNS BOOLEAN
...
CREATE OR REPLACE VIEW workspace.dq_triage.dq_config_v_rule_registry_current
...
WITH versioned AS (
  ...
  FROM workspace.dq_triage.dq_config_rule_registry r
)
SELECT * EXCEPT (rn)
FROM   versioned
WHERE  rn = 1
...
CREATE OR REPLACE VIEW workspace.dq_triage.dq_results_v_cohort_current
...
CREATE OR REPLACE VIEW workspace.dq_triage.dq_results_v_disposition_integrity
...
FROM workspace.dq_triage.dq_results_cohort c
JOIN workspace.dq_triage.dq_results_disposition d
...
FROM workspace.dq_triage.dq_results_disposition d
WHERE d.event_type = 'executed'
  AND NOT EXISTS (
    SELECT 1 FROM workspace.dq_triage.dq_results_disposition a
...
FROM workspace.dq_triage.dq_results_cohort c
WHERE NOT EXISTS (
  SELECT 1 FROM workspace.dq_triage.dq_results_disposition d
...
CREATE OR REPLACE VIEW workspace.dq_triage.dq_config_v_cde_registry_current
...
WITH versioned AS (
  ...
  FROM workspace.dq_triage.dq_config_cde_registry c
)
SELECT * EXCEPT (rn)
FROM   versioned
WHERE  rn = 1
...
CREATE OR REPLACE VIEW workspace.dq_triage.dq_results_v_cde_coverage
...
WITH bound AS (
  SELECT
    ...
    b.discovered_by
  FROM workspace.dq_triage.dq_config_v_cde_registry_current c
...
latest_run AS (
  SELECT MAX(run_ts) AS run_ts FROM workspace.dq_triage.dq_results_check_run
),
verdicts AS (
  SELECT k.rule_id, k.status, k.violation_count
  FROM   workspace.dq_triage.dq_results_check_run k
...
profiled AS (
  SELECT * FROM (
    ...
    FROM workspace.dq_triage.dq_results_cde_profile p
  ) WHERE rn = 1
)
SELECT
  g.cde_id,
