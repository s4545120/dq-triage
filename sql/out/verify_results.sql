...
FROM   workspace.dq_triage.dq_results_check_run
WHERE  run_ts = (SELECT max(run_ts) FROM workspace.dq_triage.dq_results_check_run)
GROUP  BY run_id;
...
FROM   workspace.dq_triage.dq_results_v_cohort_current
...
SELECT * FROM workspace.dq_triage.dq_results_v_disposition_integrity;
...
FROM   workspace.dq_triage.dq_results_v_cde_coverage
...
FROM   workspace.dq_triage.dq_results_cohort c
LATERAL VIEW explode(c.member_result_ids) e AS mid
LEFT   JOIN workspace.dq_triage.dq_results_check_run r ON r.result_id = e.mid
