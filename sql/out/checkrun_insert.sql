...
INSERT INTO workspace.dq_triage.dq_results_check_run
...
UNION ALL
  SELECT 'CTCT_KEY_UNIQUE' AS rule_id, 'uniqueness' AS shape, 'active' AS reg_status,
         ...
         1000 AS exp_scanned, 0 AS exp_violations
  FROM   (SELECT count(*) OVER (PARTITION BY CTCT_KEY) > 1 AS dup FROM workspace.dq_triage.dq_mock_ctct_c)
UNION ALL
...
UNION ALL
  SELECT 'CTCT_PREF_LANG_VARIANCE' AS rule_id, 'variance' AS shape, 'shadow' AS reg_status,
         count(*) AS rows_scanned,
         CASE WHEN (SELECT count(DISTINCT PREF_LANG_NM) FROM workspace.dq_triage.dq_mock_ctct_c) <= 1 THEN count(*) ELSE 0 END AS violation_count,
...
UNION ALL
  SELECT 'CTCT_SPCL_CARE_VARIANCE' AS rule_id, 'variance' AS shape, 'active' AS reg_status,
         count(*) AS rows_scanned,
         CASE WHEN (SELECT count(DISTINCT SPCL_CARE_STTS) FROM workspace.dq_triage.dq_mock_ctct_c) <= 1 THEN count(*) ELSE 0 END AS violation_count,
         1000 AS exp_scanned, 1000 AS exp_violations
  FROM   workspace.dq_triage.dq_mock_ctct_c
UNION ALL
...
UNION ALL
  SELECT 'SUBS_KEY_UNIQUE' AS rule_id, 'uniqueness' AS shape, 'active' AS reg_status,
         ...
         count_if(dup) AS violation_count,
         1000 AS exp_scanned, 0 AS exp_violations
  FROM   (SELECT count(*) OVER (PARTITION BY SUBS_KEY) > 1 AS dup FROM workspace.dq_triage.dq_mock_subs_c)
UNION ALL
...
UNION ALL
  ...
  FROM   (SELECT count(*) OVER (PARTITION BY PRIM_RSRC_VALU_TXT) > 1 AS dup FROM workspace.dq_triage.dq_mock_subs_c
...
meta (rule_id, rule_version, source_layer, target_table, target_column,
    ...
    ('CTCT_BRTH_PARSEABLE', 1, 'L2', 'workspace.dq_triage.dq_mock_ctct_c', 'BRTH_TS', 0.0, 'P1_block', 'Customer', 'dq-stewards-customer', 'active'),
    ...
    ('XREF_SUBS_CTCT_ORPHAN', 1, 'L2', 'workspace.dq_triage.dq_mock_subs_c', NULL, 0.0, 'P1_block', 'Customer', 'dq-stewards-customer', 'active')
)
