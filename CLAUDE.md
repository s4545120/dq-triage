...
| `dq-app/` | Current. Spec v1.0, redesigned 2026-09-16. Runs on the fixture **and** against Unity Catalog; deployed as the Databricks App `dq-triage`. |
| `notebooks/` | Current. The triage job's advice endpoint. **Executed** on `workspace.dq_triage` against `system.ai.gpt-oss-120b`. |

**There is a real workspace and it is loaded.** `workspace.dq_triage`, prefix `dq_` —
eight tables, five views, the two mock source tables, 1360 check runs, 1026 violation
samples and the 65-event register. The procedure is `RUNBOOK-personal-workspace.md`
and the rendered SQL is `sql/out/`, regenerated by `sql/render.py` and `sql/seed.py`.
Anything in this file that still says "never executed" is stale; fix it rather than
working around it.
...
**It split the pair.** The fixture's COH-B covers `SUBS_IMEI_NOT_NULL` and
...
answer and the mechanical grouping cannot reach it. That is a finding about the
grouping signals, not about the model.
...
the detail page exists to let a steward overrule. Worth watching before tuning the
prompt: a model that never reaches for `neither` is the failure `notebooks/README.md`
item 4 asks you to check for.

**Advice against a loaded register looks nothing like advice against an empty one.**
Six of the seven came back with a `prior_state` other than `none` — `deferred`,
`closed_rejected`, `approved_awaiting_execution`, `closed_verified` — and each said
what had already been tried. This is the spec's worked failure, avoided.
